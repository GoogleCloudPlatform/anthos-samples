...
if [ "$EXITCODE" -ne 0 ]; then
    echo "[✖️] Failed to create Anthos on bare metal cluster!"
    exit $EXITCODE
fi

KUBECONFIG_PATH="$DIR/bmctl-workspace/$CLUSTER_ID/$CLUSTER_ID-kubeconfig"
echo ""
echo "[✔️] Anthos on bare metal installation complete!"
echo "[✔️] Run [export KUBECONFIG=$KUBECONFIG_PATH] to set the kubeconfig"
echo "[✔️] Run the [$DIR/login.sh] script to generate a token that you can use to login to the cluster from the Google Cloud Console"
echo ""
...
echo "[✔️] Configuring the istio ingress for public access..."
# read the necessary variables from the init.vars file
echo "[✔️] Extracting necessary variables from the init.vars file"
...
echo "[✔️] Fetching the NodePort of the istio-ingress service in the cluster"
NODEPORT=$(kubectl \
...
echo "[✔️] Firewall ports to be updated: $FIREWALL_PORTS"

# create a patch file to update the istio-ingress service with the LB IP
echo "[✔️] Creating the patch file for istio-ingress at: /tmp/ingress-patch.yaml"
...
echo "[✔️] Patching the istio-ingress service with the external ip: $INGRESS_LB_IP"
kubectl patch \
    --kubeconfig "$KUBECONFIG_PATH" \
...
echo "[✔️] Deploying the Point of Sale application manifests"
kubectl apply \
    --kubeconfig "$KUBECONFIG_PATH" \
...
echo "[✔️] Deploying the ingress resource for the Point of Sale application"
kubectl apply \
    --kubeconfig "$KUBECONFIG_PATH" \
...
echo "[✔️] Updating the Network Endpoint Group (NEG) of the Ingress loadbalancer with the controlPlaneIP:nodePort"
for host in ${CONTROLPLAN_VM_NAMES//|/ }
do
    echo "[✔️] Adding network endpoint [instance=$host,port=$NODEPORT] to NEG group [$INGRESS_NEG]"
...
echo "[✔️] Updating firewall rule allow traffic to the ingress nodePort: $FIREWALL_PORTS"
